...
// Hitable abstraction, so we can use it as an interface
// later on.
class Material;
...
bool Scene::hit(const Ray &ray, float tMin, float tMax,
...
inline bool Sphere::hit(const Ray &ray, float tMin,
                        float tMax,
                        hitRecord &record) const {
  ...
  float b =
      2.0 *
      dot(oc, ray.direction()); // All this, is the expanded
                                // vectorized formula for a
                                // sphere centered at the
                                // origin of a radius R;
  float c =
      dot(oc, oc) -
      radius *
          radius; //                                     X^2
