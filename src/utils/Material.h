...
/* These are here because of linting issues. kur za cpp
 * lintera */
#ifndef VECTOR_H              //
...
inline bool refract(const Vector &vec, const Vector &n,
  ...
  float dotN = dot(uv, n);
  float discriminant =
      1.0 - sqr(ni_over_nt) * (1 - sqr(dotN));

  if (discriminant > 0) {
    refracted = ni_over_nt * (uv - n * dotN) -
                n * sqrt(discriminant);
    return true;
  } else {
    return false;
...
class Material {
public:
  virtual bool scatter(const Ray &rayIn, hitRecord &record,
                       Vector &attenuation,
...
public:
  ...
  virtual bool scatter(const Ray &rayIn, hitRecord &record,
    ...
    scattered = Ray(
        record.p, reflected + fuzz * randomInUnitSphere());
    attenuation = albedo;
...
public:
  ...
  virtual bool scatter(const Ray &rayIn, hitRecord &record,
    ...
    attenuation = Vector(
        1.0, 1.0,
        1.0); // Basically what colors does the dielectric
              // material absorb (0,0,0) means all
    ...
    if (dot(rayIn.direction(), record.normal) > 0) {
      ...
      cosine = refraction *
               dot(rayIn.direction(), record.normal) /
               rayIn.direction().length();
    } else {
      outward_normal = record.normal;
      ni_over_nt = 1.0 / refraction;
      cosine = -dot(rayIn.direction(), record.normal) /
               rayIn.direction().length();
    }

    if (refract(rayIn.direction(), outward_normal,
                ni_over_nt, refracted)) {
      reflect_probability =
          schlickApprox(cosine, refraction);
    } else {
...
inline Vector calculateColorVec(const Ray &ray,
  ...
  if (world->hit(ray, 0.001, MAXFLOAT, record)) {
    ...
    if (depth < RAY_DEPTH &&
        record.matPtr->scatter(ray, record, attenuation,
                               scattered)) {
      return attenuation *
             calculateColorVec(scattered, world, depth + 1);
    } else {
      return Vector(0, 0, 0);
    }
    ...
    float t = 0.5 * (unitDirection.y +
                     1.0); // hack to get T between 0 and 1
                           // so I can use it for colors

    // lerp
    return (1.0f - t) * Vector(1.0, 1.0, 1.0) +
           t * Vector(0.5, 0.7, 1.0);
  }
}
